...
export function filterOwnedArmor(
...
export function searchOwnedArmor(armor: OwnedArmorItem[], query: string): OwnedArmorItem[] {
...
export function sortOwnedArmor(armor: OwnedArmorItem[]): OwnedArmorItem[] {
  return [...armor].sort((a, b) => a.name.localeCompare(b.name));
}

function facetWithField(
  armor: OwnedArmorItem[],
  select: (a: OwnedArmorItem) => string | undefined,
  predicate: (a: OwnedArmorItem) => boolean = () => true,
): FacetOption[] {
...
export function collectOwnedArmorFacets(
  armor: OwnedArmorItem[],
): Record<string, FacetOption[]> {
  const armor30 = (a: OwnedArmorItem) => a.isArmor30 === true;
  return {
